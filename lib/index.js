...
import { isValidKey, readApiKey, writeApiKey } from './credentials.js'
...
import { effectiveMaxAmount, HISTORY_LIMIT, refusalForReferences, registerRoutes } from './routes.js'
 ...
 * @returns the model id, the parameters to quote with, whether the catalog chose,
 *   and the chosen model's own `modes` / `capabilities` (empty when the catalog
 *   could not be read), so callers can enforce the model's limits.
 */
async function resolveModelAndParams(options) {
  ...
  try {
    ...
    return { model: explicit, params: { ...DEFAULT_PARAMS, ...overrides }, catalogPicked: false, modes: [], capabilities: {} }
  }
  ...
  return {
    ...
    // Carried out so the callers can enforce the model's own limits — the
    // reference-image ceiling is per model, and the id alone cannot answer it.
    modes: Array.isArray(chosen.modes) ? chosen.modes : [],
...
function resolveSpec(raw) {
  ...
  const credentialsDir = readString(config.credentialsFile)
    ? resolveConfigDirFromFile(config.credentialsFile)
    // Deliberately the *home* directory, not `$DSH_HOME`: the sibling
    // `jws-api-demo` skill stores the same key in `~/.config/jws-image`, and
    // the whole point of that decision is that one key serves both. Using
    // `$DSH_HOME` would silently split them wherever the two differ. The env
    // override mirrors the skill script's own `configDir` option, and is what
    // lets a test pin the lookup away from the operator's real key.
    : readString(process.env.JWS_IMAGE_CONFIG_DIR) ?? join(homedir(), '.config', 'jws-image')
  return {
    ...
    // A re-pinned snapshot sits beside the key, matching §17.6's storage note.
    snapshotCacheFile: join(credentialsDir, 'openapi-snapshot.json'),
    // The history survives restarts, so its thumbnails keep resolving.
    historyFile: join(credentialsDir, 'history.json'),
 ...
 *
 * A snapshot re-pinned by the window's "更新快照" action lives in the config
 * directory and wins over the packaged one: the package ships the baseline, the
 * user's directory carries what they last accepted.
 *
 * @param spec - the resolved deployment spec.
 * @returns the snapshot, or `undefined`.
 */
async function readSnapshot(spec) {
  if (spec?.snapshotCacheFile !== undefined) {
    const cached = await readApiCache(spec.snapshotCacheFile)
    if (cached !== undefined && Array.isArray(cached.endpoints)) return cached
  }
  try {
...
/**
 * Persist a re-pinned contract snapshot.
 *
 * Best-effort like the other caches: a read-only home directory must not turn a
 * successful refresh into a failure, it just means the package baseline stays
 * in force.
 *
 * @param file - the snapshot cache path.
 * @param value - the snapshot to store.
 */
async function writeSnapshot(file, value) {
  await writeApiCache(file, value)
}

/**
 * Read the generation history from disk.
 *
 * Without this the history dies with the gateway process, which would make the
 * window's thumbnails and previews useless in practice: every restart would
 * leave a list of entries whose images can no longer be resolved.
 *
...
/**
 * Write the generation history to disk.
 *
 * Best-effort: losing the history is not worth failing a generation that has
...
function apply(ctx, rawConfig) {
  ...
  // Shared with the browser routes, so the window and the tool report the same
  // history without a second source of truth.
  const history = []
  // Restore before either entry point reads or mutates the array. In particular,
  // a fast tool call must not overwrite stored entries while this read is pending.
  const historyReady = readHistory(spec.historyFile).then((stored) => {
    if (history.length === 0) history.push(...stored.slice(0, HISTORY_LIMIT))
  })
  const persistHistory = async (entries) => {
    await historyReady
    await writeHistory(spec.historyFile, entries)
  }
  const routeDeps = {
    spec,
    ...
    historyReady,
    persistHistory,
    readApiKey: () => readApiKey({ env: process.env, configDir: spec.credentialsDir }),
    writeApiKey,
    isValidKey,
    createClient: (apiKey) => createJwsClient({ baseURL: spec.baseURL, apiKey }),
    runGeneration,
    resolveModelAndParams,
    checkApiStatus,
    readSnapshot: () => readSnapshot(spec),
    writeSnapshot,
    readApiCache,
    writeApiCache,
    writeFileAtomic,
  ...
  /**
   * Mount the browser routes once Connection exists.
   *
   * `connection` is not in this plugin's `inject` list on purpose: that would
   * stop the whole plugin — including the agent tool — from loading in a
   * headless deployment that has no web connection. `ctx.inject` waits for the
   * service without gating activation.
   */
  const mountRoutes = (scope) => {
    const count = registerRoutes(scope, routeDeps)
    if (count > 0) ctx.logger?.info('jws-image: registered %d browser routes', count)
  }
  if (typeof ctx.inject === 'function') ctx.inject(['connection'], mountRoutes)
  else mountRoutes(ctx)
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      references: { type: 'array', items: { type: 'string' }, description: 'Absolute paths of reference images. The count is checked against the chosen model\'s capabilities.maxInputImages (0/1/5/9/10/15/16 in the live catalog); a model whose modes omit image-to-image takes none. Exceeding either is refused, not truncated.' },
      quoteOnly: { type: 'boolean', description: 'Only price the request; do not generate.' },
      maxAmount: { type: 'number', description: 'A tighter per-call budget, in the configured budget currency. Clamped to the configured ceiling: it can lower the cap for one call, never raise it. Note the API quotes in USD while the configured budget is written in CNY; the two are compared without conversion.' },
    ...
    async execute(args) {
      ...
      const snapshot = await readSnapshot(spec)
      ...
      // A per-call cap may only tighten the configured ceiling, never raise it.
      const maxAmount = effectiveMaxAmount(args.maxAmount, spec.maxAmount)
      ...
      // The reference ceiling is per model, so it can only be checked once the
      // model is resolved. Refusing is the point: dropping the extras silently
      // would bill the user for a picture built from fewer images than asked.
      const refusal = refusalForReferences(chosen, references.length)
      if (refusal !== undefined) {
        return { status: 'reference-limit', message: refusal, model: chosen.model, images: [], apiLine }
      }
...
export { apply, checkApiStatus, inject, name, readHistory, readSnapshot, renderResult, resolveModelAndParams, resolveSpec, writeHistory, writeSnapshot }