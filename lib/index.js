...
import { effectiveMaxAmount, refusalForReferences, registerRoutes } from './routes.js'
...
function apply(ctx, rawConfig) {
  ...
  const history = []
  // Restore what earlier runs produced, so the window's history and its
  // thumbnails keep resolving across a gateway restart.
  readHistory(spec.historyFile).then((stored) => {
    if (stored.length > 0 && history.length === 0) history.push(...stored)
  })
  const routeDeps = {
    spec,
    budgetCurrency: BUDGET_CURRENCY,
    history,
    persistHistory: (entries) => writeHistory(spec.historyFile, entries),
