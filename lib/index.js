...
/** Fallback per-call budget in CNY (Global Constraints). */
const DEFAULT_MAX_AMOUNT = 20
/** Fallback model: the live catalog is consulted when the tool omits one. */
const DEFAULT_MODEL = 'image:seedream-4.5'
/** SKU defaults, chosen to match the cheapest documented 1K square output. */
...
function resolveSpec(raw) {
  ...
  return {
    ...
    defaultModel: readString(config.defaultModel) ?? DEFAULT_MODEL,
...
function renderResult(_args, value) {
  ...
  if (value.status === 'ok') {
    ...
    lines.push(`报价 ${formatCost(value.amount, value.currency)} 超过本次预算 ${value.maxAmount} CNY，已熔断，未提交生成。`)
...
function apply(ctx, rawConfig) {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      model: { type: 'string', description: 'Model id from the live catalog (image:...). Defaults to the configured model.' },
      params: { type: 'object', additionalProperties: true, description: 'SKU parameters: size, resolution, quality, count.' },
      ...
      maxAmount: { type: 'number', description: 'Per-call budget override.' },
    ...
    async execute(args) {
      ...
      const result = await runGeneration({
        ...
        args: {
          ...args,
          model: readString(args.model) ?? spec.defaultModel,
          params: { ...DEFAULT_PARAMS, ...(args.params ?? {}) },
          references,
      ...
      if (result.overBudget === true) {
        return {
          status: 'over-budget',
          message: `报价 ${formatCost(result.amount, result.currency)} 超过本次预算 ${result.maxAmount} CNY，已熔断，未提交生成。`,
...
export { apply, checkApiStatus, inject, name, renderResult }