...
function createRouteHandlers(deps) {
  ...
  return {
    ...
    /**
     * Drop one history entry, or the whole list.
     *
     * Only the record is touched — never the files on disk. That is deliberate:
     * the images are already paid for, and a mis-click on "清空历史" must not be
     * able to destroy them. Anyone who wants the bytes gone deletes them from
     * the output directory, which stays the single owner of the files.
     */
    async historyDelete(request) {
      ...
      if (removed === 0) return fail(404, '历史里没有匹配的记录。')
      ...
      return json({ ok: true, removed, entries: history })
    },
