...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function HistoryList(props) {
      const { entries, onRerun, onPreview, onDelete, onClear, session, useInput } = props
      useLanguage()
    ...
    function WindowBody(props) {
       ...
       * what a concurrent write did.
       */
      async function deleteHistory(body) {
        setHistoryError(null)
        const result = await call('/history-delete', { method: 'POST', body })
        if (!result.ok) {
